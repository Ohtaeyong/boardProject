...
public class Ex04 {
    ...
    @Test
    void test1() {
        List<BoardData> items = repository.findAll(Sort.by(desc("seq"), asc("subject"))); // 1차, 2차정렬(order by 추가)
    ...
    @Test
    void test2() {
        Pageable pageable = PageRequest.of(2, 3); // 첫페이지는 1이아니라 0
    ...
    void test5() {
        ...
        List<BoardData> items = repository.findByCreatedAtBetween(now.minusDays(1), now.plusDays(1), pageable); // 어제, 내일
